...
tabela `evolution_api."Webhook"` (Prisma). Um `docker restart cante-cds-evolution`
**não** perde a configuração — o Evolution relê a DB ao iniciar e re-regista o
...
```bash
source .env
curl -X POST "http://127.0.0.1:8088/webhook/set/cante35192830041502bc4b" \
...
FROM "Number"
WHERE phone = '+351928300415';
```

## Estado atual (2026-07-08)
- Instância: `cante35192830041502bc4b` (criada por curl durante debug do LID).
- Webhook: configurado manualmente para `MESSAGES_UPSERT` → `http://cante-cds-ingress:8001/channels/{number_id}/webhook`.
